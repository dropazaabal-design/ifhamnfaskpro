...
function boot( root: HTMLElement ) {
	...
	function trustPanel( scales: ScaleResult[], flags: { text: string }[], savedAt: string, shaky: boolean ) {
		...
		if ( calibrated.length ) {
			...
			items.push( '<li><span class="i">📏</span><span>الشريط البرتقالي حول الإبرة هو <b>النطاق المرجّح</b>: لو أعدت الاختبار، ففي نحو 9 من 10 مرّات تقع درجتك الحقيقية داخله. يُحسب من ثبات المقياس المنشور — كلما قلّ ثباته اتّسع النطاق.</span></li>' );
		} else {
