...
if ( root ) {
	...
	const money = ( n: number ) => toArabicDigits( Math.round( n ).toLocaleString( 'en-US' ) ).replace( /,/g, '٬' );
			...
			.map( ( l, i ) => `
				<div style="border:1px solid var(--color-border);border-radius:14px;padding:13px 15px;background:var(--color-surface)">
					<div style="display:flex;justify-content:space-between;align-items:baseline;gap:10px">
						<b style="font-size:15px">${ l.name } <span style="font-size:12px;color:var(--color-text-muted)">${ toArabicDigits( split[ i ] ) }٪</span></b>
