...
function mp_pairs( $t ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	return array(
		...
		array( 'accent stars on card', $t['accent'], $t['surface'], 3.0 ),
		array( 'accent stars on page', $t['accent'], $t['bg'], 3.0 ),
		array( 'accent on inverse', $t['accent-ink'], $t['inverse'], 3.0 ),
		array( 'inverse text on inverse', $t['inverse-ink'], $t['inverse'], 4.5 ),
		array( 'sale badge + computed', matjar_pro_readable_foreground( $t['sale'], '#FFFFFF', $t['ink'] ), $t['sale'], 4.5 ),
		array( 'sale ink on card', $t['sale-ink'], $t['surface'], 4.5 ),
